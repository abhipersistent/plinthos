 ...
 */
package org.plinthos.queue.routing;

import org.apache.log4j.BasicConfigurator;
import org.apache.log4j.Logger;
import org.plinthos.util.math.Constants;
import org.plinthos.util.math.V3d;
...
public class BirthDeathRK4 {
	...
	int timeInterval = Constants.ZERO_INT;
	...
	public BirthDeathRK4(int dof, int ntotal, int timeInterval) {
		...
		//
		ic[0] = Constants.ONE_DOUBLE;

		for (int i = 1; i < dof; i++) {
			ic[i] = Constants.ZERO_DOUBLE;
		}
		// -----------------------------------------------------------
	...
	public double[] solve(double[] lambda, double[] mu) {
		...
		for (int i = 0; i < dof; i++) {
			...
			if (i > 0 && i < dof - 1) {
				...
				coef[i] = new V3d(Constants.ZERO_DOUBLE, -(lambda[i] + mu[i]), mu[i]);

			} else if (i == dof - 1) {

				coef[i] = new V3d(lambda[i - 1], -(lambda[i] + mu[i]), Constants.ZERO_DOUBLE);
		...
		// Two dummy variables.
		double prb = Constants.ZERO_DOUBLE;
		...
		for (int i = 0; i < ntotal; i++) {
			...
			try {
				...
				for (int osc = 0; osc < dof; osc++) {
					...
					if (osc > 0 && osc < dof - 1) {
						...
						double sumP = Constants.ZERO_DOUBLE;
						...
						prb = dt
								* ((Constants.ONE_DOUBLE - sumP) + coef[osc].getX1() * ic[osc - 1] + coef[osc].getX2()
										* ic[osc]);
					}
			...
			try {
				...
				for (int osc = 0; osc < dof; osc++) {
					...
					if (osc > 0 && osc < dof - 1) {
						...
						prb = dt
								* ((Constants.ONE_DOUBLE - sumP) + coef[osc].getX1()
			...
			try {
				...
				for (int osc = 0; osc < dof; osc++) {
					...
					if (osc > 0 && osc < dof - 1) {
						...
						prb = dt
								* ((Constants.ONE_DOUBLE - sumP) + coef[osc].getX1() * k3[osc - 1] + coef[osc].getX2()
										* k3[osc]);
					}
			...
			for (int osc = 0; osc < dof; osc++) {
				...
				ic[osc] += Constants.ONE_OVER_SIX * (k1[osc] + (2.0 * k2[osc]) + (2.0 * k3[osc]) + k4[osc]);
